...
/**
 * Run a dependency audit against a ComfyUI server.
 * Returns the audit result object directly (does not write to res).
 */
export async function runDependencyAudit({ serverUrl, classTypes, modelInputs, fileInputs }) {
...
export function createDependencyAuditRouter() {
  ...
  router.post('/servers/dependency-audit', async (req, res) => {
    try {
      ...
      const result = await runDependencyAudit({ serverUrl, classTypes, modelInputs: modelInputs ?? [], fileInputs: fileInputs ?? [] });
      res.json(result);
    } catch (error) {
